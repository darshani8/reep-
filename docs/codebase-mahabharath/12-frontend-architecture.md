...
That is the complete client identity model. There is **no token field, no `localStorage`, no JWT decode, and no expiry tracking anywhere in the class.** The twelve-hour TTL lives only on the server — `SESSION_TTL_SECONDS = 60 * 60 * 12` at [apps/api-py/app/platform/credentials.py:21](apps/api-py/app/platform/credentials.py#L21), applied as the cookie's `max_age` at [api/account/sign_in.py:75](apps/api-py/app/api/account/sign_in.py#L75) and as the JWT's `exp` at [platform/credentials.py:47](apps/api-py/app/platform/credentials.py#L47). The client has no idea when its session lapses. `null` means "not known to be signed in" and is never distinguished from "known to be signed out".
...
against `GET /student/offers`, whose `response_model` ([student.py:738](apps/api-py/app/api/student/self_service.py#L738)) is snake_case ([apps/api-py/app/api/student/self_service.py:674-684](apps/api-py/app/api/student/self_service.py#L674)):
...
> **Doc/code drift, verified.** The guard's own header ([auth.guard.ts:5-7](apps/web/src/app/core/auth.guard.ts#L5)) says it "asks the backend to verify the session cookie (GET /api/auth/session)". No such endpoint exists — the auth router declares only `POST /login` ([api/account/sign_in.py:43](apps/api-py/app/api/account/sign_in.py#L43)), `GET /me` ([api/account/sign_in.py:80](apps/api-py/app/api/account/sign_in.py#L80)) and `POST /logout` ([api/account/sign_in.py:85](apps/api-py/app/api/account/sign_in.py#L85)) — and `AuthService.refresh()` correctly calls `/auth/me` ([auth.service.ts:44-49](apps/web/src/app/core/auth.service.ts#L44), whose own comment says "FastAPI exposes this as /auth/me"). The guard comment is stale migration-era wording.
...
- **Doc/code drift I verified but cannot explain.** The `app.config.ts`, `auth.service.ts` and `session.ts` headers all still name "the NestJS backend"; `auth.guard.ts` names `GET /api/auth/session`, an endpoint that does not exist (`apps/api-py/app/api/account/sign_in.py` declares only `/login`, `/me` and `/logout`). Both discrepancies are confirmed; whether they are intentional historical notes or simply un-updated, I cannot tell.
