...
after `app/interview/offline_engine.py` reached main importing numpy undeclared — the
...
## Where things live — the backend is grouped by STAKEHOLDER (2026-08)

`app/routers/` is gone. The HTTP layer is now `app/api/<stakeholder>/`, named
for **the person who calls the endpoint**, so "where does a new mentor screen's
endpoint go" has one answer and no one has to read a router to find out who it
serves. The words are the ones the `Role` enum already uses — STUDENT, MENTOR,
DIRECTOR, ALUMNI — so there is exactly one name per stakeholder across the API,
the session claims, and `apps/web/src/app/features/`.

```
app/
  main.py  config.py  db.py

  api/                     the HTTP layer, by who calls it
    student/               self_service, programme, badges,
                           interview_session, interview_records
    mentor/                mentees, student_records, notebook,
                           badge_review, own_certificates, leave
    director/              programme_dashboard
    alumni/                profile_and_jobs
    account/               sign_in, registration        (every role)
    system/                health                       (the load balancer)
    legacy/                text_assistant, voice_assistant

  platform/                machinery every stakeholder relies on; NO domain rules
                           identity, access_rules, credentials, google_sign_in,
                           request_id, audit_trail, rate_limit, redaction,
                           document_store, mailer
  interview/               realtime_relay, offline_engine, specializations,
                           audio_store
  assistant/               knowledge_base, conversations, tools
  reports/                 resume_pdf, english_baseline_pdf   (local render only)

  models/  schemas/  ai/  workers/  eval/     unchanged locations
  seed*.py  worker.py  retention_job.py  grant_access.py
                           operator entrypoints — `python -m app.seed_kb` and
                           friends are a PUBLISHED interface (deploy.yml,
                           ops-task.yml, infra/aws/grant.sh). NOT renamed.
```

**A module lives under the stakeholder who CALLS it.** Where one serves two, it
lives with the owner of the data and `main.py` says so — `api/student/
interview_records.py` is the one case: the student reads their own interviews at
`/api/interview/*`, and staff read the same rows, rule-2 gated, at
`/api/mentor/*`, from `staff_router` in the same file.

Two renames worth knowing because the old names said nothing:
`app/architecture_events.py` → `platform/audit_trail.py`; and the old
`models/redesign.py` — 24 classes over four unrelated concerns under a
project-phase name — split into
`models/tenant.py`, `models/audit.py`, `models/notebook.py` and
`models/knowledge_versioned.py`. **Table names did not change** (`redesign_*`),
so none of that was a migration. `app/memory.py` was a tombstone and is deleted.

This move changed **no route, no table and no test**: the same 161 operations are
served by the same handler functions, and the same 623 tests collect. The files
still hold what they held — `api/student/self_service.py` is the old 2 422-line
`routers/student.py`, unsplit. Splitting it along the names above is the next
commit, not this one.
...
**Staff read these through rule 2's gate**, in `app/api/mentor/student_records.py`:
...
through `_assert_can_access_student` — imported from `api/mentor/mentees.py`, never
...
OWN completed-course certificates, `app/api/mentor/own_certificates.py`). The
...
is `app/api/alumni/profile_and_jobs.py`: `GET /api/alumni/profile` answers `created: false`
