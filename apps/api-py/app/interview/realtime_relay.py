...
microphone. Nothing here imports app.assistant.tools, app.assistant.knowledge_base or any ORM
...
and the database live in app/api/student/interview_session.py, which is the only caller.
...
# -- alone for the generic interview, or composed by app/interview/specializations.py
...
# from app/interview/audio_store.py (see the `recorder` note in __init__). They MUST
...
# before an upstream connection exists. Raised by api/student/interview_session.py's
...
# api/student/interview_session.py only AFTER the client is posting consent rows, or every
...
class _TurnWriteRefused(Exception):
    ...
    Raised by the turn writer in api/student/interview_session.py when
...
class _ConnectionLimiter:
    ...
    Deliberately NOT api/legacy/voice_assistant.py's `_TOKEN_GRANTS.try_acquire(user_id,
    ...
    (api/legacy/voice_assistant.py's version DOES take a lock because it is called from
...
class _RelaySession:
    ...
    def __init__(
        ...
        # model, no Session and no app.assistant.conversations -- rule 1's containment is
        # structural, not a convention -- so everything it writes leaves through
        # a callable api/student/interview_session.py supplies. All four are SYNCHRONOUS and
        # allowed to block: this class runs each on a worker thread via
        # asyncio.to_thread, because app/assistant/conversations.py is synchronous
        ...
        # api/student/interview_session.py from app.interview.audio_store.recorder_for(), and it is
        ...
        #
        # Duck-typed, and this module does NOT import app.interview.audio_store: that
    ...
    async def _finalize_session(self, code: int, reason: str) -> None:
        ...
        await self._write_record(
            ...
            _SessionOutcome(
                ...
                # scope_store_audio grant (app/interview/audio_store.py), and the
    ...
    async def run(self) -> tuple[int, str]:
        ...
        try:
            ...
            # still find by session id (app/interview/audio_store.py names it that way
