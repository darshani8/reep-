...
app/platform/google_sign_in.py:redirect_uri() from google_auth.CALLBACK_PATH, and documented
...
that every consumer still reads â app/platform/identity.py, require_mentor,
_assert_can_access_student, the WebSocket auth in app/api/student/interview_session.py, and
...
A SESSION CAN NOW BE REVOKED, within the limits app/platform/credentials.py states plainly.
...
from app.models.user import LoginDay, User
from app.schemas.auth import LoginRequest, SessionUser
from app.platform.credentials import (
    SESSION_COOKIE,
    SESSION_TTL_SECONDS,
...
def _payload_for(user: User) -> dict:
    ...
    # logged out. app/platform/credentials.py reads an absent claim as version 0 â it has to,
    ...
    # key in the claim set that app/platform/identity.py, the interview WebSocket, the Angular
...
def logout(
    ...
    kills it: app/platform/credentials.py refuses any token carrying a version behind the
    ...
    if claims:
        ...
        try:
            ...
            log.exception(
                ...
                "expires (see users.token_version in app/platform/credentials.py)",
                user_id,
            )
