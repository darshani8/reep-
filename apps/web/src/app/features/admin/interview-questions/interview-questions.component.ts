...
export class InterviewQuestionsComponent {
  ...
  async add(): Promise<void> {
    ...
    await this.mutate(async () => {
      const res = await this.post(`${environment.apiBase}/admin/interview-questions`, {
        track: this.track(),
        phase: this.fPhase(),
        text,
      });
  ...
  async bulkAdd(): Promise<void> {
    ...
    await this.mutate(async () => {
      const res = await this.post(`${environment.apiBase}/admin/interview-questions/bulk`, {
        track: this.track(),
        lines,
      });
      if (!res.ok) throw new Error(await this.detailOf(res));
  ...
  async remove(q: Question): Promise<void> {
    ...
    await this.mutate(async () => {
      const res = await fetch(`${environment.apiBase}/admin/interview-questions/${q.id}`, {
        method: 'DELETE',
        credentials: 'include',
      });
      if (!res.ok) throw new Error(await this.detailOf(res));
  ...
  async move(q: Question, dir: -1 | 1): Promise<void> {
    ...
    await this.mutate(async () => {
      const res = await this.post(`${environment.apiBase}/admin/interview-questions/reorder`, {
        track: this.track(),
        ids: list.map((x) => x.id),
      });
      if (!res.ok) throw new Error(await this.detailOf(res));
  ...
  private async loadTracks(): Promise<void> {
    try {
      const res = await fetch(`${environment.apiBase}/admin/interview-questions/tracks`, { credentials: 'include' });
  ...
  private async loadQuestions(): Promise<void> {
    ...
    try {
      const res = await fetch(
        `${environment.apiBase}/admin/interview-questions?track=${encodeURIComponent(this.track())}`,
        { credentials: 'include' },
      );
      if (!res.ok) throw new Error(String(res.status));
      this.questions.set((await res.json()) as Question[]);
