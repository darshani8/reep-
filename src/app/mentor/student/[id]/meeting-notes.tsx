...
/**
 * The mentor action bar, with the meeting timestamp on it.
 *
 * Four things a mentor can do about what they have just read, and all four
 * really write to the database through the Server Actions in `./actions.ts`.
 * Every one of them leaves a note, so nothing happens to a student off the
 * record.
 *
 * The field that earns this component its own file is **when the meeting
 * happened**. `MentorNote.createdAt` is when the row was written and is not
 * editable — that is what makes it evidence. But the note a mentor types on
 * Monday morning is usually about Friday afternoon, and a log that dates that
 * conversation to Monday is quietly wrong about the one fact a mentor comes back
 * for. So `meetingAt` defaults to now, which is right for a note typed in the
 * room, and is editable for the two cases that break that assumption: writing up
 * a past meeting, and booking a future one.
 *
 * Only one of the four actions is loud. Writing a note is the thing a mentor
 * does after a conversation, so it gets the accent; flagging, nudging and
 * booking a 1:1 are one-tap side effects and sit underneath as plain text. A row
 * of four equally weighted buttons reads as a control panel, which is exactly
 * the tone this screen has to avoid.
 */
...
export function MeetingNotePanel({
  studentId,
  studentName,
  /// `YYYY-MM-DDTHH:mm` in server-local time. Computed on the server and passed
  /// in, because a `new Date()` evaluated during a client render disagrees with
  /// the one the server already rendered and hydration warns about it.
  defaultMeetingAt,
  onFlag,
  onNudge,
  onSchedule,
  ...
  // stays exactly where the mentor left it, and so does the meeting time — a
  // rejected save that also reset the timestamp to "now" would silently turn a
  // write-up of Friday into a note about today.
  useEffect(() => {
    if (noteResult?.ok) {
      setNoteText('');
      setMeetingAt(defaultMeetingAt);
    }
  }, [noteResult, defaultMeetingAt]);
        ...
        */}
        <Box component="form" action={saveNote}>
          ...
          <Stack
            ...
            spacing={2}
            sx={{ mt: 2, alignItems: { sm: 'center' }, maxWidth: '72ch' }}
          >
            <TextField
              label="When you met"
              name="meetingAt"
            ...
            <Typography variant="caption" color="text.secondary">
              Defaults to now. Move it back for a meeting you are writing up later, or
              forward for one you have booked — the note records when you typed it either
              way.
            </Typography>
          </Stack>
