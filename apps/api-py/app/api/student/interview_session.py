...
The relay engine itself is app/interview/realtime_relay.py; this module is the boundary:
...
assistant screen's entry point — see the header on app/api/legacy/text_assistant.py, which
...
(app/api/account/sign_in.py), which is also why a cross-site page cannot carry it onto
...
sole thing sent upstream is the fixed persona in app/interview/realtime_relay.py plus the
student's microphone. Nothing here imports app.assistant.tools, app.assistant.knowledge_base or
...
use, through app/assistant/conversations.py, so GET /api/agent/history returns them
...
recorder is constructed (app.interview.audio_store.recorder_for), and it hands back
...
from app.platform.identity import get_current_session, get_ws_session
from app.interview.audio_store import recorder_for
from app.interview.specializations import Specialization, get_specialization
...
def _make_turn_writer(conversation_id: str, interview_session_id: str):
    ...
    Synchronous on purpose: app/assistant/conversations.py is synchronous SQLAlchemy, and
    ...
    def write(
        ...
        try:
            ...
            # app/interview/realtime_relay.py imports no ORM model and no database code at
...
async def interview(websocket: WebSocket) -> None:
    ...
    try:
        ...
        # SELECT on a cache miss (platform/credentials.py says so itself), and this
    ...
    # app/interview/realtime_relay.py imports no ORM model and no database code at all,
    ...
    if settings.interview_engine.strip().lower() == "local":
        from app.interview.offline_engine import LocalSession
