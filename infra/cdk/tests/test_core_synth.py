...
def test_harden_renders_tls_and_an_alert_subscriber(hardened: Template) -> None:
    """THE LIVE STACK MUST BE REPRODUCIBLE FROM THIS REPOSITORY ALONE.

    INCIDENT (2026-09-15). Seven context values the deployed stack depends on
    were passed as `-c` flags at deploy time and recorded NOWHERE — not in
    cdk.json, not in the runbook. A synth from the repository therefore rendered
    a DIFFERENT STACK: one plain HTTP listener where the live ALB has an HTTPS
    listener plus a redirect, and no SNS subscription where the live topic has
    the only address anyone is alerted on.

    That template synthesises cleanly and passes every other guard here. Nothing
    said it was wrong. Pressing any `core-*` button in cdk-deploy.yml would have
    deleted TLS and silenced every alarm, and the diff that says so is only
    visible against the LIVE stack, which CI cannot reach.

    So the guard is the shape instead: a harden-phase synth, using the
    repository's own cdk.json, must render the things the live stack has. It is
    the same lesson as tools/tfvars_from_state.py — "the first apply's values
    were typed as -var flags and recorded nowhere, and without them the plan can
    never be clean" — caught here rather than in production.
    """
    listeners = hardened.find_resources("AWS::ElasticLoadBalancingV2::Listener")
    ports = sorted(l["Properties"]["Port"] for l in listeners.values())
    assert ports == [80, 443], (
        f"expected an HTTP listener and an HTTPS listener, got ports {ports}. "
        "A cert-less synth renders ONE plain HTTP listener, and deploying it "
        "DELETES the live HTTPS listener — check albAcmCertificateArn in cdk.json"
    )
    https = [l for l in listeners.values() if l["Properties"]["Port"] == 443]
    assert https, "no HTTPS listener"
    assert https[0]["Properties"].get("Certificates"), "the HTTPS listener carries no certificate"
    assert https[0]["Properties"]["Protocol"] == "HTTPS"

    # The alert topic with nobody subscribed is an alarm that fires into a void.
    hardened.resource_count_is("AWS::SNS::Subscription", 1)
    subs_ = hardened.find_resources("AWS::SNS::Subscription")
    assert any(s["Properties"]["Protocol"] == "email" for s in subs_.values()), (
        "no email subscriber on the alerts topic — check alertEmail in cdk.json"
    )


def test_every_deploy_time_context_value_is_recorded() -> None:
    """The seven keys by name, so a future edit cannot quietly drop one.

    Listed explicitly rather than derived: the failure mode is a value that
    exists in the deployed stack and in nobody's memory, and a test that reads
    the same file it is checking would pass on an empty file.
    """
    import json
    from pathlib import Path

    ctx = json.loads((Path(__file__).resolve().parents[1] / "cdk.json").read_text())["context"]
    for key in (
        "domainName",
        "albOriginDomain",
        ...
        assert ctx.get(key), (
            f"cdk.json is missing {key}. It is a value the LIVE stack depends on; "
            "without it a synth renders a different stack and deploying it destroys "
            "what the live one has. Recover it from the live resource, do not guess."
        )
