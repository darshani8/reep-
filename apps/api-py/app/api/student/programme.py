...
ITS OWN MODULE, so `app/api/student/self_service.py` — already 2 200 lines and the file
...
a different endpoint on `api/mentor/mentees.py`, behind `_assert_can_access_student`,
...
from app.reports.english_baseline_pdf import render_english_report_pdf
from app.models.timesheet import DayActivity
from app.models.user import Mentor, Student, User

router = APIRouter(prefix="/student", tags=["student-screens"])


def _require_student(session: dict) -> str:
    """The same gate `api/student/self_service.py` uses, deliberately duplicated rather
...
def compose_english_baseline(db: Session, student_id: str) -> EnglishBaselineOut:
    ...
    through `api/mentor/student_records.py`. Two hand-written copies of "is this
