 ...
 *
 * One list, one editor. `GET /director/swoc` is every student with every entry
 ...
 * 1–5 weight — with an input box to add one; `POST /director/swoc/{id}`,
 * `PATCH` and `DELETE /director/swoc/entries/{id}` do the writing. The route
...
@Component({
  selector: 'app-director-swoc',
  standalone: true,
  imports: [],
  templateUrl: './swoc.component.html',
  styleUrl: './swoc.component.scss',
})
export class DirectorSwocComponent {
  ...
  async load(): Promise<void> {
    this.error.set(null);
    try {
      const res = await fetch(`${environment.apiBase}/director/swoc`, { credentials: 'include' });
  ...
  async add(kind: Kind): Promise<void> {
    ...
    await this.run(async () => {
      const res = await fetch(`${environment.apiBase}/director/swoc/${row.student_id}`, {
        method: 'POST',
        credentials: 'include',
  ...
  async remove(entry: Entry): Promise<void> {
    ...
    await this.run(async () => {
      const res = await fetch(`${environment.apiBase}/director/swoc/entries/${entry.id}`, {
        method: 'DELETE',
        credentials: 'include',
      });
