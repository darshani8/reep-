...
def test_login_success_sets_session_cookie(client):
    r = client.post("/auth/login", json={"email": "student@bgscet.ac.in", "password": "student123"})
...
def test_login_wrong_password_401(client):
    r = client.post("/auth/login", json={"email": "student@bgscet.ac.in", "password": "nope"})
...
def test_student_can_read_own_dashboard(client, login):
    ...
    assert client.get("/student/dashboard", headers=h).status_code == 200


@requires_db
def test_student_forbidden_from_mentor_area(client, login):
    h = login("student@bgscet.ac.in", "student123")
    assert client.get("/mentor/mentees", headers=h).status_code == 403


@requires_db
def test_student_forbidden_from_director_area(client, login):
    h = login("student@bgscet.ac.in", "student123")
    assert client.get("/director/overview", headers=h).status_code == 403
...
def test_mentor_forbidden_from_director_only(client, login):
    ...
    assert client.get("/mentor/mentees", headers=h).status_code == 200
    assert client.get("/director/alert-rules", headers=h).status_code == 403


@requires_db
def test_director_can_read_overview(client, login):
    h = login("director@bgscet.ac.in", "director123")
    assert client.get("/director/overview", headers=h).status_code == 200


@requires_db
def test_unauthenticated_is_rejected(client):
    # No cookie → the session dependency refuses (this app's convention is 403).
    assert client.get("/student/dashboard").status_code in (401, 403)
...
def test_resume_generate_respects_egress_gate(client, login):
    ...
    h = login("student@bgscet.ac.in", "student123")
    r = client.post("/student/resume/generate", headers=h, json={"title": "T"})
    assert r.status_code == 200
