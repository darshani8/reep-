...
`POST /api/auth/login` (password) is **the second door, and it is shut unless something opens it.** The `login` fixture in `tests/conftest.py` and the test modules that post to it authenticate through it, so deleting it would take the DB-backed suite and CI with it. The guard is `settings.password_login_allowed = _is_dev_env(env) or local_auth_ready` — still an allowlist of dev/CI environment names, now OR the explicit `LOCAL_AUTH_ENABLED=true` opt-in **with a ready email transport**, and still never `not is_prod`: an unrecognised, blank or typo'd `ENV` with nothing configured refuses exactly as before, and the only way it opens anywhere outside dev is the same opt-in that opens it in production. The flag alone does not open it and a transport alone does not either; when shut it answers 403 and names Google. A student sets, resets or changes a password through **two endpoints and one screen** (`/login/password?mode=create|reset|change`): `POST /api/auth/password/otp {email}` emails a 6-digit code and `POST /api/auth/password/set {email, code, new_password}` signs them in — only for a row that already exists on the roster AND whose address is on `ROSTER_EMAIL_DOMAIN` (`local_auth.address_allowed`); nothing self-provisions and no role changes. `/otp` answers **202 with one constant body for everyone** — enrolled, unknown, off-domain, throttled, capped, or the send failed — and the handler touches no database before the response: the whole distinguishing path (domain fence, lookup, cooldown, hourly cap, insert, send) runs in a `BackgroundTasks` callable with its own `SessionLocal()` after the 202 is written, so neither body nor clock enumerates the roster. An optional `reep_session` cookie on either endpoint is bound to the submitted address (403 on a mismatch, before the DB), which is what lets the signed-in *change* flow reuse the anonymous endpoints. Codes live 10 minutes, 5 wrong guesses, 3 per hour, 60 s between requests; stored only as an HMAC keyed on `AUTH_SECRET`; **setting a password bumps `users.token_version` and revokes the user's every other session** — the new cookie is the one survivor. `LOCAL_AUTH_ENABLED` needs `EMAIL_TRANSPORT` = `ses` (signed by the task role, no key), `smtp`, or `log` (dev/CI only: it prints the code into the API log, so `email_ready` refuses it as a transport on every non-dev `ENV`); blank on a dev `ENV` means `log`. A transport that fails at runtime keeps `/login` up and makes `/otp` answer 202 while the ERROR line `auth-otp send failed`, the FAILED `mail_logs` row and the CloudWatch alarm say what happened — the runbook query is `select status, error from mail_logs where kind='auth-otp' order by sent_at desc limit 20;`, and the full design record and troubleshooting table are in `docs/email-password-sign-in.md`. Set `GOOGLE_CLIENT_ID` / `GOOGLE_CLIENT_SECRET` in `apps/api-py/.env`; blank means `GET /api/auth/sso/status` reports unavailable and the login screen's Google button renders disabled with the reason — nothing else is affected. Full notes, including the exact authorised redirect URI and a troubleshooting section: `docs/google-sign-in.md`.
