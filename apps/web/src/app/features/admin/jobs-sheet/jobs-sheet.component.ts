...
export class AdminJobsSheetComponent {
  ...
  async publish(): Promise<void> {
    ...
    try {
      const res = await fetch(`${environment.apiBase}/admin/jobs`, {
        method: 'POST',
        credentials: 'include',
  ...
  async remove(row: JobRow): Promise<void> {
    ...
    this.error.set(null);
    try {
      const res = await fetch(`${environment.apiBase}/admin/jobs/${row.id}`, {
        method: 'DELETE',
        credentials: 'include',
      });
      if (!res.ok) {
  ...
  private async load(): Promise<void> {
    try {
      const res = await fetch(`${environment.apiBase}/admin/jobs`, { credentials: 'include' });
      if (!res.ok) {
